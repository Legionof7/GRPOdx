...
Fixed: Now we override _prepare_inputs(...) so each training step 
       actually runs multi_turn_generation(...) and obtains a reward.
"""
...
########################################
# 2. (Same GPT-4o Patient & Judge Functions)
########################################
...
########################################
# 4. The Trainer: override _prepare_inputs
########################################
...
class DoctorWithGpt4oTrainer(UnslothGRPOTrainer):
    """
    The critical piece is to override _prepare_inputs(...) so that 
    we actually call multi_turn_generation(...) for each prompt and get a final reward.
    """
    ...
    def multi_turn_generation(self, prompt, model, tokenizer, generation_config, max_new_tokens=50, game_object=None):
        ...
        # We'll just return dummy token IDs (not the real conversation) 
        # because we do multi-turn calls step by step
        completion_ids = [0,1,2]
        return completion_ids, final_reward

    def _prepare_inputs(self, inputs: dict):
        """
        Overriding the method so each "prompt" triggers a multi-turn conversation,
        then we compute advantage with the final reward from GPT-4o Judge.
        """
        ...
        for example in inputs:
            # if it's a list of role/content, we might do:
            text = ""
        ...
        # We'll create some dummy tokenization, because we won't actually 
        # generate with it. We'll do multi_turn_generation manually
        prompt_ids = torch.tensor([[0]], device=device)
        prompt_mask = torch.tensor([[1]], device=device)

        # If we're using vLLM, we gather all prompts and run them in the main process
        if self.args.use_vllm:
            ...
            # We only have 1 prompt in this example, but let's handle batch anyway
            completion_ids_tensors = []
            for cids in completion_ids_list:
                completion_ids_tensors.append(torch.tensor(cids, device=device))
            # pad
            from trl.trainer.grpo_trainer import pad
            ...
            # Not using vLLM => not recommended for multi-turn in this code
            raise NotImplementedError("Must use vLLM = True for multi-turn in this snippet.")

        # We skip the standard log-likelihood calculation for reference model, or do it trivially
        # We'll still produce advantage to keep the pipeline consistent
        # For a single reward function, plus game_reward

        # We'll create a dummy 'ref_per_token_logps' of the same shape
        ...
        # We gather the final reward across processes
        game_rewards_tensor = gather(game_rewards_tensor)

        # We can combine it with a dummy reward_func if you want
        # Here we skip that and just do advantage = reward - mean
        final_rewards = game_rewards_tensor

        # If we have num_generations>1, we'd group them, etc.
        # We'll do a simpler approach: advantage=0 for everyone but we do 
        ...
        # We might replicate the shape to match the number of completions
        # In a real code, you'd do the grouping if you do multiple completions per prompt
        # We'll keep it at shape [batch_size]
        ...
        # Return RL data
        return {
            "prompt_ids": prompt_ids,  # shape (1,1)
            "prompt_mask": prompt_mask,  # shape (1,1)
            "completion_ids": completion_ids_padded,  # shape (B, length)
            "completion_mask": completion_mask,  # shape (B, length)
            "ref_per_token_logps": ref_per_token_logps,  # shape (B, length)
            "advantages": advantages,  # shape (B,)
        }
